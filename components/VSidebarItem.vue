<template>
  <li>
    <div class="flex items-center justify-between p-2 rounded hover:bg-blue-700 select-none">
      <div class="flex items-center space-x-2 w-full">
        <IconAdapter v-if="item.icon" :name="item.icon" />

        <template v-if="item.to">
          <VLink :to="item.to" :external="item.external" class="flex-1">
            {{ item.label }}
          </VLink>
        </template>
    ...
    <ul v-if="isGroup && open" class="pl-6 space-y-1 mt-1">
      <VSidebarItem v-for="child in item.children" :key="child.label" :item="child" />
    </ul>
  </li>
</template>
